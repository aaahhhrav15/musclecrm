...
router.get('/download', async (req, res) => {
  try {
    // Fetch the gym information
    const gymId = req.user?.gymId;
    if (!gymId) {
      return res.status(400).json({ success: false, message: 'Gym ID not found' });
    }
    ...
    // Use gym name or fallback to default
    const gymName = gym.name || 'FITNESS CENTER GYM';
    ...
    // Header - CENTER ALIGNED with gym name
    doc.fillColor('#2c3e50');
    currentY = addCenteredText(doc, gymName.toUpperCase(), currentY, 18);
...
I hereby release, waive, and discharge ${gymName}, its owners, employees, and agents from any liability, claims, or damages arising from my participation in activities or use of facilities.
...
I certify that I am in good physical condition and have no medical conditions that would prevent safe participation. I agree to follow all facility rules and safety guidelines.`;
    ...
    console.error('Error generating waiver form:', error);
    if (!res.headersSent) {
      res.status(500).json({ success: false, message: 'Error generating waiver form' });
    }
  }
});
