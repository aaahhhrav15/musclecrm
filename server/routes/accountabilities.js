...
router.get('/', auth, gymAuth, async (req, res) => {
  try {
    ...
    // Get accountabilities from existing collection with gym and date filter
    console.log('Accountabilities filter:', JSON.stringify(filter, null, 2));
...
router.get('/:id', auth, gymAuth, async (req, res) => {
  try {
    ...
    const accountability = await db.collection('accountabilities').findOne({ _id: new mongoose.Types.ObjectId(id), gymId });
